      ...
      r = function (t) {
        ...
        if ((s != s && (s = 0), !(s < 0 || s >= n))) {
          ...
          return i >= 55296 &&
            i <= 56319 &&
            n > s + 1 &&
            ((u = a.charCodeAt(s + 1)), u >= 56320 && u <= 57343)
            ? (i - 55296) * 1024 + u - 56320 + 65536
            : i
        }
      }
    e
...
function ba(e) {
  ...
  if (
    ((r = e.source[e.sourceIndex + 1]),
    (r = 256 * r + e.source[e.sourceIndex]),
    (t = e.source[e.sourceIndex + 3]),
    (t = 256 * t + e.source[e.sourceIndex + 2]),
    r !== (~t & 65535))
  )
    return xt
...
function Sa(e, r) {
  ...
  return t.destLen < t.dest.length
    ? typeof t.dest.slice == 'function'
      ? t.dest.slice(0, t.destLen)
      : t.dest.subarray(0, t.destLen)
    : t.dest
}
ma(bt, St)
Ft(Tr, kr, 4, 3)
...
function Te(e, r, t, a, n) {
  return (
    Math.pow(1 - n, 3) * e +
    3 * Math.pow(1 - n, 2) * n * r +
    3 * (1 - n) * Math.pow(n, 2) * t +
    Math.pow(n, 3) * a
  )
}
function pe() {
...
pe.prototype.addBezier = function (e, r, t, a, n, s, i, u) {
  ...
  for (var p = 0; p <= 1; p++) {
    ...
    if (d === 0) {
      ...
      0 < m &&
        m < 1 &&
        (p === 0 && this.addX(Te(o[p], l[p], f[p], h[p], m)),
        p === 1 && this.addY(Te(o[p], l[p], f[p], h[p], m)))
      continue
    }
    ...
    if (!(y < 0)) {
      ...
      0 < C &&
        C < 1 &&
        (p === 0 && this.addX(Te(o[p], l[p], f[p], h[p], C)),
        p === 1 && this.addY(Te(o[p], l[p], f[p], h[p], C)))
      var S = (-c - Math.sqrt(y)) / (2 * d)
      0 < S &&
        S < 1 &&
        (p === 0 && this.addX(Te(o[p], l[p], f[p], h[p], S)),
        p === 1 && this.addY(Te(o[p], l[p], f[p], h[p], S)))
    }
  }
}
pe.prototype.addQuad = function (e, r, t, a, n, s) {
  var i = e + 0.6666666666666666 * (t - e),
    u = r + (2 / 3) * (a - r),
    o = i + (1 / 3) * (n - e),
    l = u + (1 / 3) * (s - r)
...
P.prototype.extend = function (e) {
  if (e.commands) e = e.commands
    ...
    this.moveTo(r.x1, r.y1),
      this.lineTo(r.x2, r.y1),
      this.lineTo(r.x2, r.y2),
      this.lineTo(r.x1, r.y2),
      this.close()
    return
  }
...
P.prototype.getBoundingBox = function () {
  for (
    var e = new pe(), r = 0, t = 0, a = 0, n = 0, s = 0;
    ...
    switch (i.type) {
      ...
      case 'Q':
        ...
        break
      case 'C':
        e.addBezier(a, n, i.x1, i.y1, i.x2, i.y2, i.x, i.y),
          (a = i.x),
          (n = i.y)
        break
      case 'Z':
...
P.prototype.toPathData = function (e) {
  ...
  for (var a = '', n = 0; n < this.commands.length; n += 1) {
    ...
    s.type === 'M'
      ...
      ? (a += 'L' + t(s.x, s.y))
      : s.type === 'C'
      ? (a += 'C' + t(s.x1, s.y1, s.x2, s.y2, s.x, s.y))
      : s.type === 'Q'
      ? (a += 'Q' + t(s.x1, s.y1, s.x, s.y))
      : s.type === 'Z' && (a += 'Z')
  }
  return a
}
...
g.BYTE = function (e) {
  return (
    U.argument(e >= 0 && e <= 255, 'Byte value should be between 0 and 255.'),
    [e]
  )
}
E.BYTE = ae(1)
...
g.TAG = function (e) {
  return (
    U.argument(e.length === 4, 'Tag should be exactly 4 ASCII characters.'),
...
g.INDEX = function (e) {
  ...
  for (
    var i = [],
      u = (1 + Math.floor(Math.log(r) / Math.log(2)) / 8) | 0,
      o = [void 0, g.BYTE, g.USHORT, g.UINT24, g.ULONG][u],
      l = 0;
    l < t.length;
    l += 1
  ) {
...
g.OPERAND = function (e, r) {
  ...
  if (Array.isArray(r))
    for (var a = 0; a < r.length; a += 1)
      U.argument(
        e.length === r.length,
        'Not enough arguments given for type' + r
      ),
        (t = t.concat(g.OPERAND(e[a], r[a])))
...
g.OBJECT = function (e) {
  var r = g[e.type]
  return (
    U.argument(r !== void 0, 'No encoding function for type ' + e.type),
    r(e.value)
  )
}
E.OBJECT = function (e) {
  var r = E[e.type]
  return (
    U.argument(r !== void 0, 'No sizeOf function for type ' + e.type),
    r(e.value)
  )
}
g.TABLE = function (e) {
  for (var r = [], t = e.fields.length, a = [], n = [], s = 0; s < t; s += 1) {
    ...
    var l = u(o)
    i.type === 'TABLE'
      ? (n.push(r.length), (r = r.concat([0, 0])), a.push(l))
      : (r = r.concat(l))
  }
  for (var f = 0; f < a.length; f += 1) {
    var h = n[f],
      p = r.length
    U.argument(p < 65536, 'Table ' + e.tableName + ' too big.'),
      (r[h] = p >> 8),
      (r[h + 1] = p & 255),
      (r = r.concat(a[f]))
  }
...
E.TABLE = function (e) {
  for (var r = 0, t = e.fields.length, a = 0; a < t; a += 1) {
    ...
    U.argument(
      s !== void 0,
      'No sizeOf function for field type ' + n.type + ' (' + n.name + ')'
    )
    var i = e[n.name]
  ...
  e.format === 1
    ...
    ? z.call(
        ...
        [{ name: 'coverageFormat', type: 'USHORT', value: 2 }].concat(
          we('rangeRecord', e.ranges, function (r) {
            return [
...
function Ze(e) {
  z.call(
    this,
    'scriptListTable',
    we('scriptRecord', e, function (r, t) {
      var a = r.script,
        n = a.defaultLangSys
      return (
        U.assert(
...
function Qe(e) {
  z.call(
    this,
    'featureListTable',
    we('featureRecord', e, function (r, t) {
      var a = r.feature
      return [
        { name: 'featureTag' + t, type: 'TAG', value: r.tag },
        {
          ...
          value: new z(
            'featureTable',
            [
              { name: 'featureParams', type: 'USHORT', value: a.featureParams }
...
function Ke(e, r) {
  z.call(
    this,
    'lookupListTable',
    dr('lookup', e, function (t) {
      var a = r[t.lookupType]
      return (
        U.assert(
          !!a,
          'Unable to write GSUB lookup type ' + t.lookupType + ' tables.'
        ),
        new z(
          'lookupTable',
          [
...
v.prototype.parseULongList = function (e) {
  ...
  for (
    var r = new Array(e),
      t = this.data,
      a = this.offset + this.relativeOffset,
      n = 0;
    n < e;
    n++
  )
...
v.prototype.parseValueRecord = function (e) {
  if ((e === void 0 && (e = this.parseUShort()), e !== 0)) {
    var r = {}
    return (
      e & 1 && (r.xPlacement = this.parseShort()),
      e & 2 && (r.yPlacement = this.parseShort()),
      e & 4 && (r.xAdvance = this.parseShort()),
      e & 8 && (r.yAdvance = this.parseShort()),
      e & 16 && ((r.xPlaDevice = void 0), this.parseShort()),
      e & 32 && ((r.yPlaDevice = void 0), this.parseShort()),
      e & 64 && ((r.xAdvDevice = void 0), this.parseShort()),
      e & 128 && ((r.yAdvDevice = void 0), this.parseShort()),
      r
    )
  }
}
v.prototype.parseValueRecordList = function () {
  for (
    var e = this.parseUShort(), r = this.parseUShort(), t = new Array(r), a = 0;
    a < r;
    a++
  )
...
v.prototype.parseListOfLists = function (e) {
  for (
    ...
    if (((this.relativeOffset = i), e)) {
      for (
        var u = this.parseOffset16List(), o = new Array(u.length), l = 0;
        l < u.length;
        l++
      )
    ...
    this.parsePointer(
      v.recordList({
        ...
        script: v.pointer({
          ...
          langSysRecords: v.recordList({ tag: v.tag, langSys: v.pointer(zr) })
        })
      })
    ) || []
  )
    ...
    this.parsePointer(
      v.list(
        v.pointer(function () {
          ...
          U.argument(
            1 <= r && r <= 9,
            'GPOS/GSUB lookup type ' + r + ' unknown.'
          )
...
function Aa(e, r) {
  ...
  for (var a = 0; a < t; a += 1)
    for (
      var n = r.parseULong(), s = r.parseULong(), i = r.parseULong(), u = n;
      u <= s;
      u += 1
    )
...
function Ba(e, r, t, a, n) {
  ...
  var s
  ;(e.segCount = s = r.parseUShort() >> 1),
    r.skip('uShort', 3),
    (e.glyphIndexMap = {})
  for (
    var i = new k.Parser(t, a + n + 14),
      u = new k.Parser(t, a + n + 16 + s * 2),
      o = new k.Parser(t, a + n + 16 + s * 4),
      l = new k.Parser(t, a + n + 16 + s * 6),
      f = a + n + 16 + s * 8,
      h = 0;
    h < s - 1;
    h += 1
...
function Ia(e, r) {
  var t = {}
  ;(t.version = k.getUShort(e, r)),
    U.argument(t.version === 0, 'cmap table version should be 0.'),
    (t.numTables = k.getUShort(e, r + 2))
  for (var a = -1, n = t.numTables - 1; n >= 0; n -= 1) {
    var s = k.getUShort(e, r + 4 + n * 8),
      i = k.getUShort(e, r + 4 + n * 8 + 2)
    if (
      (s === 3 && (i === 0 || i === 1 || i === 10)) ||
      (s === 0 && (i === 0 || i === 1 || i === 2 || i === 3 || i === 4))
    ) {
...
function Ga(e) {
  ...
  var n = [
    ...
    { name: 'offset', type: 'ULONG', value: r ? 12 : 12 + 8 }
  ]
  r ||
    (n = n.concat([
  ...
  for (t = 0; t < o; t += 1) {
    ...
    m.end <= 65535 && m.start <= 65535
      ? ((f = f.concat({ name: 'end_' + t, type: 'USHORT', value: m.end })),
        (h = h.concat({ name: 'start_' + t, type: 'USHORT', value: m.start })),
        (p = p.concat({ name: 'idDelta_' + t, type: 'SHORT', value: m.delta })),
        (c = c.concat({
      ...
      !r &&
        ...
        (x = x.concat({ name: 'cmap12End_' + t, type: 'ULONG', value: m.end })),
        (x = x.concat({
          name: 'cmap12Glyph_' + t,
          type: 'ULONG',
          value: m.glyphIndex
  ...
  if (
    ((s.segCountX2 = (o - l) * 2),
    (s.searchRange =
      Math.pow(2, Math.floor(Math.log(o - l) / Math.log(2))) * 2),
  ...
  switch (e.version) {
    ...
    case 2:
      ...
      for (var r = 0; r < e.numberOfGlyphs; r++)
        e.glyphNameIndex[r] < xe.length
          ? (this.names[r] = xe[e.glyphNameIndex[r]])
          : (this.names[r] = e.names[e.glyphNameIndex[r] - xe.length])
      break
    case 2.5:
      this.names = new Array(e.numberOfGlyphs)
...
function za(e) {
  for (
    var r, t = e.tables.cmap.glyphIndexMap, a = Object.keys(t), n = 0;
    n < a.length;
    n += 1
  ) {
    var s = a[n],
  ...
  for (var u = 0; u < e.glyphs.length; u += 1)
    (r = e.glyphs.get(u)),
      e.cffEncoding
        ? e.isCIDFont
          ? (r.name = 'gid' + u)
          : (r.name = e.cffEncoding.charset[u])
        : e.glyphNames.names && (r.name = e.glyphNames.glyphIndexToName(u))
}
function Wa(e) {
  e._IndexToUnicodeMap = {}
  for (
    var r = e.tables.cmap.glyphIndexMap, t = Object.keys(r), a = 0;
    a < t.length;
    a += 1
  ) {
    var n = t[a],
      s = r[n]
    e._IndexToUnicodeMap[s] === void 0
      ? (e._IndexToUnicodeMap[s] = { unicodes: [parseInt(n)] })
...
Q.prototype.bindConstructorValues = function (e) {
  ;(this.index = e.index || 0),
    ...
    Object.defineProperty(this, 'path', qa(this, e.path))
}
...
Q.prototype.getPath = function (e, r, t, a, n) {
  ...
  var u = a.xScale,
    o = a.yScale
  if (
    (a.hinting && n && n.hinting && (i = this.path && n.hinting.exec(this, t)),
    i)
  )
    (s = n.hinting.getCommands(i)),
      (e = Math.round(e)),
      (r = Math.round(r)),
      (u = o = 1)
  else {
  ...
  for (var f = new P(), h = 0; h < s.length; h += 1) {
    var p = s[h]
    p.type === 'M'
      ? f.moveTo(e + p.x * u, r + -p.y * o)
      : p.type === 'L'
      ? f.lineTo(e + p.x * u, r + -p.y * o)
      : p.type === 'Q'
      ? f.quadraticCurveTo(
          e + p.x1 * u,
          r + -p.y1 * o,
...
Q.prototype.getMetrics = function () {
  for (
    var e = this.path.commands, r = [], t = [], a = 0;
    a < e.length;
    a += 1
  ) {
    var n = e[a]
    n.type !== 'Z' && (r.push(n.x), t.push(n.y)),
      (n.type === 'Q' || n.type === 'C') && (r.push(n.x1), t.push(n.y1)),
      n.type === 'C' && (r.push(n.x2), t.push(n.y2))
  }
  var s = {
...
Q.prototype.drawPoints = function (e, r, t, a) {
  function n(h, p, c, d) {
    ...
    for (var x = 0; x < h.length; x += 1)
      e.moveTo(p + h[x].x * d, c + h[x].y * d),
        e.arc(p + h[x].x * d, c + h[x].y * d, 2, 0, Math.PI * 2, !1)
    e.closePath(), e.fill()
  }
  ;(r = r !== void 0 ? r : 0),
    (t = t !== void 0 ? t : 0),
    (a = a !== void 0 ? a : 24)
  for (
    var s = (1 / this.path.unitsPerEm) * a,
      i = [],
      u = [],
      o = this.path,
      l = 0;
    ...
    var f = o.commands[l]
    f.x !== void 0 && i.push({ x: f.x, y: -f.y }),
      f.x1 !== void 0 && u.push({ x: f.x1, y: -f.y1 }),
      f.x2 !== void 0 && u.push({ x: f.x2, y: -f.y2 })
  }
...
Q.prototype.drawMetrics = function (e, r, t, a) {
  ...
  ;(r = r !== void 0 ? r : 0),
    ...
    (e.lineWidth = 1),
    (e.strokeStyle = 'black'),
    ye.line(e, r, -1e4, r, 1e4),
    ye.line(e, -1e4, t, 1e4, t)
  var s = this.xMin || 0,
    i = this.yMin || 0,
    u = this.xMax || 0,
    o = this.yMax || 0,
  ...
  ;(e.strokeStyle = 'blue'),
    ye.line(e, r + s * n, -1e4, r + s * n, 1e4),
    ye.line(e, r + u * n, -1e4, r + u * n, 1e4),
    ye.line(e, -1e4, t + -i * n, 1e4, t + -i * n),
    ye.line(e, -1e4, t + -o * n, 1e4, t + -o * n),
    (e.strokeStyle = 'green'),
    ye.line(e, r + l * n, -1e4, r + l * n, 1e4)
}
function _e(e, r, t) {
...
Cr.prototype.get = function (e) {
  if (this.glyphs[e] === void 0) {
    ...
    this.font.cffEncoding
      ? this.font.isCIDFont
        ? (r.name = 'gid' + e)
        : (r.name = this.font.cffEncoding.charset[e])
      : this.font.glyphNames.names &&
        (r.name = this.font.glyphNames.glyphIndexToName(e)),
...
function gr(e) {
  var r
  return (
    e.length < 1240 ? (r = 107) : e.length < 33900 ? (r = 1131) : (r = 32768), r
  )
}
function ve(e, r, t) {
  var a = [],
...
function $a(e) {
  for (var r = {}, t = 0; t < e.length; t += 1) {
    var a = e[t][0],
      ...
      s = void 0
    if (
      (n.length === 1 ? (s = n[0]) : (s = n),
      r.hasOwnProperty(a) && !isNaN(r[a]))
    )
...
function wt(e, r, t) {
  ...
  for (t = t !== void 0 ? t : e.length; a.relativeOffset < t; ) {
    var i = a.parseByte()
    i <= 21
      ? (i === 12 && (i = 1200 + a.parseByte()), n.push([i, s]), (s = []))
      : s.push(ja(a, i))
  }
  return $a(n)
}
  ...
  for (var a = {}, n, s = 0; s < r.length; s += 1) {
    ...
    if (Array.isArray(i.type)) {
      ...
      for (var o = 0; o < i.type.length; o++)
        (n = e[i.op] !== void 0 ? e[i.op][o] : void 0),
          n === void 0 &&
            (n =
              i.value !== void 0 && i.value[o] !== void 0 ? i.value[o] : null),
          i.type[o] === 'SID' && (n = Oe(t, n)),
          (u[o] = n)
      a[i.name] = u
    } else
      (n = e[i.op]),
        n === void 0 && (n = i.value !== void 0 ? i.value : null),
        i.type === 'SID' && (n = Oe(t, n)),
        (a[i.name] = n)
  }
  return a
...
function en(e, r) {
  var t = {}
  return (
    (t.formatMajor = k.getCard8(e, r)),
    (t.formatMinor = k.getCard8(e, r + 1)),
    (t.size = k.getCard8(e, r + 2)),
    (t.offsetSize = k.getCard8(e, r + 3)),
    (t.startOffset = r),
    (t.endOffset = r + 4),
    t
  )
...
function Wr(e, r, t, a) {
  for (var n = [], s = 0; s < t.length; s += 1) {
    ...
    if (o !== 0 && l !== 0) {
      var f = It(e, l + r, o, a)
      if (
        ((u._defaultWidthX = f.defaultWidthX),
        (u._nominalWidthX = f.nominalWidthX),
        f.subrs !== 0)
      ) {
        var h = l + f.subrs,
          p = ve(e, h + r)
...
function an(e, r, t) {
  ...
  if (i === 0)
    ...
    for (var f = 0; f < l; f += 1)
      for (
        var h = s.parseCard8(), p = s.parseCard8(), c = h;
        c <= h + p;
        c += 1
      )
        (n[c] = a), (a += 1)
    ...
    for (var G, Y, Z, j, $, M, N, W, _, V, H, X, A = 0; A < F.length; ) {
      ...
      switch (((A += 1), q)) {
        ...
        case 6:
          for (
            ;
            o.length > 0 && ((p += o.shift()), u.lineTo(p, c), o.length !== 0);
        ...
        case 7:
          for (
            ;
            o.length > 0 && ((c += o.shift()), u.lineTo(p, c), o.length !== 0);
          ...
          switch (((q = F[A]), (A += 1), q)) {
            case 35:
              ;(a = p + o.shift()),
                ...
                (c = X + o.shift()),
                o.shift(),
                u.curveTo(a, n, s, i, N, W),
                u.curveTo(_, V, H, X, p, c)
              break
            case 34:
              ;(a = p + o.shift()),
                (n = c),
            ...
            default:
              console.log(
                'Glyph ' + r.index + ': unknown operator ' + 1200 + q
              ),
                (o.length = 0)
          }
          break
        case 14:
          o.length > 0 && !f && ((R = o.shift() + y), (f = !0)),
            h && (u.closePath(), (h = !1))
          break
        case 18:
          D()
        ...
        default:
          q < 32
            ? console.log('Glyph ' + r.index + ': unknown operator ' + q)
            : q < 247
            ? o.push(q - 139)
            : q < 251
            ? ((G = F[A]), (A += 1), o.push((q - 247) * 256 + G + 108))
            : q < 255
            ? ((G = F[A]), (A += 1), o.push(-(q - 251) * 256 - G - 108))
            : ((G = F[A]),
              (Y = F[A + 1]),
              (Z = F[A + 2]),
              (j = F[A + 3]),
              (A += 4),
              o.push(((G << 24) | (Y << 16) | (Z << 8) | j) / 65536))
      }
    }
  }
...
function sn(e, r, t, a) {
  ...
  var f = l[0]
  if (
    ((t.tables.cff.topDict = f),
    f._privateDict &&
      ((t.defaultWidthX = f._privateDict.defaultWidthX),
      (t.nominalWidthX = f._privateDict.nominalWidthX)),
    f.ros[0] !== void 0 && f.ros[1] !== void 0 && (t.isCIDFont = !0),
    t.isCIDFont)
  ) {
    var h = f.fdArray,
      p = f.fdSelect
  ...
  if (
    ((t.defaultWidthX = m.defaultWidthX),
    (t.nominalWidthX = m.nominalWidthX),
    m.subrs !== 0)
  ) {
    var y = x + m.subrs,
      C = ve(e, y)
  ...
  var S
  a.lowMemory
    ? ((S = Qa(e, r + f.charStrings)), (t.nGlyphs = S.offsets.length))
    : ((S = ve(e, r + f.charStrings)), (t.nGlyphs = S.objects.length))
  var R = tn(e, r + f.charset, t.nGlyphs, u.objects)
  if (
    (f.encoding === 0
      ? (t.cffEncoding = new je(Na, R))
      : f.encoding === 1
      ? (t.cffEncoding = new je(Ha, R))
      : (t.cffEncoding = an(e, r + f.encoding, R)),
    (t.encoding = t.encoding || t.cffEncoding),
    (t.glyphs = new ue.GlyphSet(t)),
    a.lowMemory)
  )
    t._push = function (L) {
...
function Mt(e, r) {
  ...
  return (
    a >= 0 && (t = a),
    (a = r.indexOf(e)),
    a >= 0 ? (t = a + qe.length) : ((t = qe.length + r.length), r.push(e)),
    t
  )
}
function on() {
...
function Pt(e, r, t) {
  for (var a = {}, n = 0; n < e.length; n += 1) {
    ...
    i !== void 0 &&
      !Rt(i, s.value) &&
      (s.type === 'SID' && (i = Mt(i, t)),
      (a[s.op] = { name: s.name, type: s.type, value: i }))
  }
  return a
}
function Vr(e, r) {
...
function hn(e) {
  ...
  for (var a = 0, n = 0, s = 0; s < t.commands.length; s += 1) {
    ...
    if (o.type === 'M')
      (i = Math.round(o.x - a)),
        (u = Math.round(o.y - n)),
        r.push({ name: 'dx', type: 'NUMBER', value: i }),
        r.push({ name: 'dy', type: 'NUMBER', value: u }),
        r.push({ name: 'rmoveto', type: 'OP', value: 21 }),
        (a = Math.round(o.x)),
        (n = Math.round(o.y))
    else if (o.type === 'L')
      (i = Math.round(o.x - a)),
        ...
        r.push({ name: 'dx', type: 'NUMBER', value: i }),
        r.push({ name: 'dy', type: 'NUMBER', value: u }),
        r.push({ name: 'rlineto', type: 'OP', value: 5 }),
        (a = Math.round(o.x)),
        (n = Math.round(o.y))
    else if (o.type === 'C') {
      ...
      ;(i = Math.round(o.x - o.x2)),
        (u = Math.round(o.y - o.y2)),
        r.push({ name: 'dx1', type: 'NUMBER', value: h }),
        r.push({ name: 'dy1', type: 'NUMBER', value: p }),
        r.push({ name: 'dx2', type: 'NUMBER', value: c }),
        r.push({ name: 'dy2', type: 'NUMBER', value: d }),
        r.push({ name: 'dx', type: 'NUMBER', value: i }),
        r.push({ name: 'dy', type: 'NUMBER', value: u }),
        r.push({ name: 'rrcurveto', type: 'OP', value: 8 }),
        (a = Math.round(o.x)),
        (n = Math.round(o.y))
    }
  }
...
function cn(e) {
  for (
    var r = new b.Record('CharStrings INDEX', [
...
function dn(e, r) {
  ...
  var f = Vr(n, l)
  ;(t.topDictIndex = qr(f)),
    (t.globalSubrIndex = fn()),
    (t.charsets = pn(i, l)),
    (t.charStringsIndex = cn(e)),
    (t.privateDict = vn(s, l)),
    (t.stringIndex = ln(l))
  var h =
    t.header.sizeOf() +
    t.nameIndex.sizeOf() +
  ...
  return (
    (n.charset = h),
    (n.encoding = 0),
    (n.charStrings = n.charset + t.charsets.sizeOf()),
    (n.private[1] = n.charStrings + t.charStringsIndex.sizeOf()),
    (f = Vr(n, l)),
    (t.topDictIndex = qr(f)),
    t
  )
...
function gn(e, r) {
  var t = {},
    a = new k.Parser(e, r)
  return (
    (t.version = a.parseVersion()),
    (t.fontRevision = Math.round(a.parseFixed() * 1e3) / 1e3),
    (t.checkSumAdjustment = a.parseULong()),
    (t.magicNumber = a.parseULong()),
    U.argument(
      t.magicNumber === 1594834165,
      'Font header has wrong magic number.'
    ),
    (t.flags = a.parseUShort()),
    (t.unitsPerEm = a.parseUShort()),
    (t.created = a.parseLongDateTime()),
    (t.modified = a.parseLongDateTime()),
    (t.xMin = a.parseShort()),
    (t.yMin = a.parseShort()),
    (t.xMax = a.parseShort()),
    (t.yMax = a.parseShort()),
    (t.macStyle = a.parseUShort()),
    (t.lowestRecPPEM = a.parseUShort()),
    (t.fontDirectionHint = a.parseShort()),
    (t.indexToLocFormat = a.parseShort()),
    (t.glyphDataFormat = a.parseShort()),
    t
  )
}
function mn(e) {
...
function yn(e, r) {
  ...
  return (
    (t.version = a.parseVersion()),
    (t.ascender = a.parseShort()),
    (t.descender = a.parseShort()),
    (t.lineGap = a.parseShort()),
    (t.advanceWidthMax = a.parseUShort()),
    (t.minLeftSideBearing = a.parseShort()),
    (t.minRightSideBearing = a.parseShort()),
    (t.xMaxExtent = a.parseShort()),
    (t.caretSlopeRise = a.parseShort()),
    (t.caretSlopeRun = a.parseShort()),
    (t.caretOffset = a.parseShort()),
    (a.relativeOffset += 8),
    (t.metricDataFormat = a.parseShort()),
    (t.numberOfHMetrics = a.parseUShort()),
    t
  )
}
function xn(e) {
...
function Sn(e, r, t, a, n) {
  ...
  for (var s, i, u = new k.Parser(r, t), o = 0; o < n; o += 1)
    o < a && ((s = u.parseUShort()), (i = u.parseShort())),
      (e._hmtxTableData[o] = { advanceWidth: s, leftSideBearing: i })
}
...
function kn(e) {
  for (var r = new b.Table('hmtx', []), t = 0; t < e.length; t += 1) {
    ...
    r.fields.push({ name: 'advanceWidth_' + t, type: 'USHORT', value: n }),
      r.fields.push({ name: 'leftSideBearing_' + t, type: 'SHORT', value: s })
  }
  return r
}
...
function Fn(e) {
  for (
    ...
    var s = t.indexOf(e[n])
    s < 0 && ((s = t.length), (t += e[n])),
      r.fields.push({ name: 'offset ' + n, type: 'USHORT', value: a + s }),
      r.fields.push({ name: 'length ' + n, type: 'USHORT', value: e[n].length })
  }
...
function Un(e, r) {
  ...
  for (var n = t.parseULong(), s = [], i = 0; i < n; i++) {
    for (
      var u = '', o = r + t.parseUShort(), l = t.parseUShort(), f = o;
      f < o + l;
      ++f
    )
...
function Cn(e, r) {
  ...
  return (
    (t.version = a.parseVersion()),
    (t.numGlyphs = a.parseUShort()),
    t.version === 1 &&
      ((t.maxPoints = a.parseUShort()),
      (t.maxContours = a.parseUShort()),
      (t.maxCompositePoints = a.parseUShort()),
      (t.maxCompositeContours = a.parseUShort()),
      (t.maxZones = a.parseUShort()),
      (t.maxTwilightPoints = a.parseUShort()),
      (t.maxStorage = a.parseUShort()),
      (t.maxFunctionDefs = a.parseUShort()),
      (t.maxInstructionDefs = a.parseUShort()),
      (t.maxStackElements = a.parseUShort()),
      (t.maxSizeOfInstructions = a.parseUShort()),
      (t.maxComponentElements = a.parseUShort()),
      (t.maxComponentDepth = a.parseUShort())),
    t
  )
}
function En(e) {
...
function Xr(e, r, t, a, n, s) {
  return new b.Record('NameRecord', [
    ...
    { name: 'nameID', type: 'USHORT', value: a },
...
function Bn(e, r) {
  ...
  f.sort(function (Z, j) {
    return (
      Z.platformID - j.platformID ||
      Z.encodingID - j.encodingID ||
      Z.languageID - j.languageID ||
      Z.nameID - j.nameID
    )
  })
  for (
    var G = new b.Table('name', [
...
function Mn(e, r) {
  var t = {},
    a = new k.Parser(e, r)
  ;(t.version = a.parseUShort()),
    (t.xAvgCharWidth = a.parseShort()),
    (t.usWeightClass = a.parseUShort()),
    (t.usWidthClass = a.parseUShort()),
    (t.fsType = a.parseUShort()),
    (t.ySubscriptXSize = a.parseShort()),
    (t.ySubscriptYSize = a.parseShort()),
    (t.ySubscriptXOffset = a.parseShort()),
    (t.ySubscriptYOffset = a.parseShort()),
    (t.ySuperscriptXSize = a.parseShort()),
    (t.ySuperscriptYSize = a.parseShort()),
    (t.ySuperscriptXOffset = a.parseShort()),
    (t.ySuperscriptYOffset = a.parseShort()),
    (t.yStrikeoutSize = a.parseShort()),
    (t.yStrikeoutPosition = a.parseShort()),
    (t.sFamilyClass = a.parseShort()),
    (t.panose = [])
  for (var n = 0; n < 10; n++) t.panose[n] = a.parseByte()
  return (
    (t.ulUnicodeRange1 = a.parseULong()),
    (t.ulUnicodeRange2 = a.parseULong()),
    (t.ulUnicodeRange3 = a.parseULong()),
    (t.ulUnicodeRange4 = a.parseULong()),
    (t.achVendID = String.fromCharCode(
      a.parseByte(),
      a.parseByte(),
      a.parseByte(),
      a.parseByte()
    )),
    (t.fsSelection = a.parseUShort()),
    (t.usFirstCharIndex = a.parseUShort()),
    (t.usLastCharIndex = a.parseUShort()),
    (t.sTypoAscender = a.parseShort()),
    (t.sTypoDescender = a.parseShort()),
    (t.sTypoLineGap = a.parseShort()),
    (t.usWinAscent = a.parseUShort()),
    (t.usWinDescent = a.parseUShort()),
    t.version >= 1 &&
      ((t.ulCodePageRange1 = a.parseULong()),
      (t.ulCodePageRange2 = a.parseULong())),
    t.version >= 2 &&
      ((t.sxHeight = a.parseShort()),
      (t.sCapHeight = a.parseShort()),
      (t.usDefaultChar = a.parseUShort()),
      (t.usBreakChar = a.parseUShort()),
      (t.usMaxContent = a.parseUShort())),
    t
  )
}
function Pn(e) {
...
function Gn(e, r) {
  ...
  switch (
    ((t.version = a.parseVersion()),
    (t.italicAngle = a.parseFixed()),
    (t.underlinePosition = a.parseShort()),
    (t.underlineThickness = a.parseShort()),
    (t.isFixedPitch = a.parseULong()),
    (t.minMemType42 = a.parseULong()),
    (t.maxMemType42 = a.parseULong()),
    (t.minMemType1 = a.parseULong()),
    (t.maxMemType1 = a.parseULong()),
    t.version)
  ) {
    case 1:
      t.names = xe.slice()
      break
    case 2:
      ;(t.numberOfGlyphs = a.parseUShort()),
        (t.glyphNameIndex = new Array(t.numberOfGlyphs))
...
ee[4] = function () {
  var r = this.parseUShort()
  return (
    U.argument(r === 1, 'GSUB ligature table identifier-format must be 1'),
    {
      substFormat: r,
      coverage: this.parsePointer(v.coverage),
...
ee[5] = function () {
  ...
  U.assert(
    !1,
    '0x' + r.toString(16) + ': lookup type 5 format must be 1, 2 or 3.'
  )
}
ee[6] = function () {
  var r = this.offset + this.relativeOffset,
  ...
  U.assert(
    !1,
    '0x' + r.toString(16) + ': lookup type 6 format must be 1, 2 or 3.'
  )
}
ee[7] = function () {
  var r = this.parseUShort()
...
var Ue = new Array(9)
Ue[1] = function (r) {
  return r.substFormat === 1
    ? new b.Table('substitutionTable', [
        { name: 'substFormat', type: 'USHORT', value: 1 },
        { name: 'coverage', type: 'TABLE', value: new b.Coverage(r.coverage) },
...
Ue[2] = function (r) {
  return (
    U.assert(r.substFormat === 1, 'Lookup type 2 substFormat must be 1.'),
    new b.Table(
      'substitutionTable',
      [
        { name: 'substFormat', type: 'USHORT', value: 1 },
        ...
        b.tableList('seqSet', r.sequences, function (t) {
...
Ue[3] = function (r) {
  return (
    U.assert(r.substFormat === 1, 'Lookup type 3 substFormat must be 1.'),
    new b.Table(
      'substitutionTable',
      [
        { name: 'substFormat', type: 'USHORT', value: 1 },
        ...
        b.tableList('altSet', r.alternateSets, function (t) {
...
Ue[4] = function (r) {
  return (
    U.assert(r.substFormat === 1, 'Lookup type 4 substFormat must be 1.'),
    new b.Table(
      'substitutionTable',
      [
        { name: 'substFormat', type: 'USHORT', value: 1 },
        ...
        b.tableList('ligSet', r.ligatureSets, function (t) {
          return new b.Table(
            'ligatureSetTable',
            b.tableList('ligature', t, function (a) {
              return new b.Table(
                'ligatureTable',
                [
                  { name: 'ligGlyph', type: 'USHORT', value: a.ligGlyph }
                ].concat(
                  b.ushortList(
                    'component',
                    a.components,
  ...
  if (r.substFormat === 1) {
    var t = new b.Table(
      ...
      [
        ...
        b.tableList('chainRuleSet', r.chainRuleSets, function (s) {
          return new b.Table(
            'chainRuleSetTable',
            b.tableList('chainRule', s, function (i) {
              var u = b
                .ushortList('backtrackGlyph', i.backtrack, i.backtrack.length)
                .concat(b.ushortList('inputGlyph', i.input, i.input.length + 1))
                .concat(
                  b.ushortList(
                    'lookaheadGlyph',
                    i.lookahead,
                ...
                .concat(
                  b.ushortList('substitution', [], i.lookupRecords.length)
                )
              return (
                i.lookupRecords.forEach(function (o, l) {
                  u = u
                    .concat({
    ...
    U.assert(!1, 'lookup type 6 format 2 is not yet supported.')
...
function zn(e) {
  return new b.Table('GSUB', [
    ...
    { name: 'lookups', type: 'TABLE', value: new b.LookupList(e.lookups, Ue) }
  ])
}
...
function Wn(e, r) {
  var t = new k.Parser(e, r),
    a = t.parseULong()
  U.argument(a === 1, 'Unsupported META table version.'),
    t.parseULong(),
    t.parseULong()
  for (var n = t.parseULong(), s = {}, i = 0; i < n; i++) {
    var u = t.parseTag(),
...
function _n(e) {
  var r = Object.keys(e).length,
    ...
    n = new b.Table('meta', [
      { name: 'version', type: 'ULONG', value: 1 },
      { name: 'flags', type: 'ULONG', value: 0 },
      { name: 'offset', type: 'ULONG', value: a },
      { name: 'numTags', type: 'ULONG', value: r }
    ])
  for (var s in e) {
    var i = t.length
    ;(t += e[s]),
      n.fields.push({ name: 'tag ' + s, type: 'TAG', value: s }),
      n.fields.push({ name: 'offset ' + s, type: 'ULONG', value: a + i }),
      n.fields.push({ name: 'length ' + s, type: 'ULONG', value: e[s].length })
  }
...
function jt(e) {
  ...
  ;(r.searchRange = 16 * t),
    (r.entrySelector = Zr(t)),
    (r.rangeShift = r.numTables * 16 - r.searchRange)
  for (
    var a = [], n = [], s = r.sizeOf() + Qr().sizeOf() * r.numTables;
    s % 4 !== 0;
  ...
  for (var i = 0; i < e.length; i += 1) {
    ...
    for (
      a.push({ name: l.tag + ' Table Record', type: 'RECORD', value: l }),
        n.push({ name: u.tableName + ' table', type: 'RECORD', value: u }),
        s += o,
        U.argument(!isNaN(s), 'Something went wrong calculating the offset.');
      s % 4 !== 0;
  ...
  return (
    ...
    (r.fields = r.fields.concat(a)),
    (r.fields = r.fields.concat(n)),
    r
  )
}
function Kr(e, r, t) {
...
function qn(e) {
  for (
    ...
    if (isNaN(x.advanceWidth))
      throw new Error(
        'Glyph ' + x.name + ' (' + d + '): advanceWidth is not a number.'
      )
    ...
    if (x.name !== '.notdef') {
      ...
      r.push(C.xMin),
        ...
        a.push(C.xMax),
        n.push(C.yMax),
        i.push(C.leftSideBearing),
        u.push(C.rightSideBearing),
        s.push(x.advanceWidth)
    }
  }
  var S = {
    xMin: Math.min.apply(null, r),
  ...
  var W = [],
    ...
    q = [R, O, D, L, _, G, H, X, F]
  V && q.push(V),
    e.tables.gsub && q.push(Kt.make(e.tables.gsub)),
    A && q.push(A)
  for (
    var rr = jt(q),
      ha = rr.encode(),
...
Ae.prototype = {
  ...
  getTable: function (e) {
    ...
    return (
      !r &&
        e &&
        (r = this.font.tables[this.tableName] = this.createDefaultTable()),
      r
    )
  },
    ...
    if (n) {
      ...
      if (a) {
        ...
        return (
          ...
          (s = { tag: t, feature: { params: 0, lookupListIndexes: [] } }),
          u.push(s),
          i.push(l),
          s.feature
        )
  ...
  getGlyphClass: function (e, r) {
    switch (e.format) {
      case 1:
        return e.startGlyph <= r && r < e.startGlyph + e.classes.length
          ? e.classes[r - e.startGlyph]
          : 0
      case 2:
        var t = jr(e.ranges, r)
...
K.prototype.getSingle = function (e, r, t) {
  for (
    var a = [], n = this.getLookupTables(r, t, e, 1), s = 0;
    s < n.length;
    s++
  )
...
K.prototype.getMultiple = function (e, r, t) {
  for (
    var a = [], n = this.getLookupTables(r, t, e, 2), s = 0;
    s < n.length;
    s++
  )
    for (var i = n[s].subtables, u = 0; u < i.length; u++) {
...
K.prototype.getAlternates = function (e, r, t) {
  for (
    var a = [], n = this.getLookupTables(r, t, e, 3), s = 0;
    s < n.length;
    s++
  )
    for (var i = n[s].subtables, u = 0; u < i.length; u++)
...
K.prototype.getLigatures = function (e, r, t) {
  for (
    var a = [], n = this.getLookupTables(r, t, e, 4), s = 0;
    s < n.length;
    s++
  )
    for (var i = n[s].subtables, u = 0; u < i.length; u++)
...
K.prototype.addSingle = function (e, r, t, a) {
  ...
  U.assert(
    s.coverage.format === 1,
    'Single: unable to modify coverage table format ' + s.coverage.format
  )
  var i = r.sub,
    u = this.binSearch(s.coverage.glyphs, i)
  u < 0 &&
    ((u = -1 - u),
    s.coverage.glyphs.splice(u, 0, i),
    s.substitute.splice(u, 0, 0)),
    (s.substitute[u] = r.by)
}
K.prototype.addMultiple = function (e, r, t, a) {
  U.assert(
    r.by instanceof Array && r.by.length > 1,
  ...
  U.assert(
    s.coverage.format === 1,
    'Multiple: unable to modify coverage table format ' + s.coverage.format
  )
  var i = r.sub,
    u = this.binSearch(s.coverage.glyphs, i)
  u < 0 &&
...
K.prototype.addAlternate = function (e, r, t, a) {
  ...
  U.assert(
    s.coverage.format === 1,
    'Alternate: unable to modify coverage table format ' + s.coverage.format
  )
  var i = r.sub,
    u = this.binSearch(s.coverage.glyphs, i)
  u < 0 &&
    ((u = -1 - u),
    s.coverage.glyphs.splice(u, 0, i),
    s.alternateSets.splice(u, 0, 0)),
    (s.alternateSets[u] = r.by)
}
K.prototype.addLigature = function (e, r, t, a) {
  ...
  if (l >= 0) {
    ...
    (l = -1 - l),
      s.coverage.glyphs.splice(l, 0, i),
      s.ligatureSets.splice(l, 0, [o])
}
...
K.prototype.add = function (e, r, t, a) {
  ...
  switch (e) {
    case 'aalt':
    case 'salt':
      return typeof r.by == 'number'
        ? this.addSingle(e, r, t, a)
        : this.addAlternate(e, r, t, a)
    case 'dlig':
    case 'liga':
    case 'rlig':
      return this.addLigature(e, r, t, a)
    case 'ccmp':
      return r.by instanceof Array
        ? this.addMultiple(e, r, t, a)
        : this.addLigature(e, r, t, a)
  }
}
function Zn() {
...
function $t(e) {
  for (
    var r = new ArrayBuffer(e.length), t = new Uint8Array(r), a = 0;
    a < e.length;
    ++a
  )
    t[a] = e[a]
...
function Qn(e) {
  for (
    var r = new Buffer(e.byteLength), t = new Uint8Array(e), a = 0;
    a < r.length;
    ++a
  )
    r[a] = t[a]
...
function ea(e, r, t) {
  ...
  ;(e.numberOfContours = a.parseShort()),
    (e._xMin = a.parseShort()),
    (e._yMin = a.parseShort()),
    (e._xMax = a.parseShort()),
    (e._yMax = a.parseShort())
  var n, s
  if (e.numberOfContours > 0) {
    for (
      var i = (e.endPointIndices = []), u = 0;
      ...
      if (l > 0) {
        for (var x = 0; x < l; x += 1)
          (s = n[x]),
            (d = {}),
            (d.onCurve = !!(s & 1)),
            (d.lastPointOfContour = i.indexOf(x) >= 0),
            c.push(d)
    ...
    for (var R = !0; R; ) {
      ...
      ;(n & 1) > 0
        ? (n & 2) > 0
          ? ((O.dx = a.parseShort()), (O.dy = a.parseShort()))
          : (O.matchedPoints = [a.parseUShort(), a.parseUShort()])
        : (n & 2) > 0
        ? ((O.dx = a.parseChar()), (O.dy = a.parseChar()))
        : (O.matchedPoints = [a.parseByte(), a.parseByte()]),
        (n & 8) > 0
          ? (O.xScale = O.yScale = a.parseF2Dot14())
          : (n & 64) > 0
          ? ((O.xScale = a.parseF2Dot14()), (O.yScale = a.parseF2Dot14()))
          : (n & 128) > 0 &&
            ((O.xScale = a.parseF2Dot14()),
            (O.scale01 = a.parseF2Dot14()),
            (O.scale10 = a.parseF2Dot14()),
            (O.yScale = a.parseF2Dot14())),
        e.components.push(O),
        (R = !!(n & 32))
    }
    if (n & 256) {
  ...
  for (var t = Kn(e), a = 0; a < t.length; ++a) {
    ...
    for (var l = 0; l < n.length; ++l)
      if (((s = i), (i = u), (u = n[(l + 1) % n.length]), i.onCurve))
        ...
        var f = s,
          h = u
        s.onCurve || (f = { x: (i.x + s.x) * 0.5, y: (i.y + s.y) * 0.5 }),
          u.onCurve || (h = { x: (i.x + u.x) * 0.5, y: (i.y + u.y) * 0.5 }),
    ...
    for (var t = 0; t < r.components.length; t += 1) {
      ...
      if ((n.getPath(), n.points)) {
        ...
        if (a.matchedPoints === void 0) s = ir(n.points, a)
          ...
          if (
            a.matchedPoints[0] > r.points.length - 1 ||
            a.matchedPoints[1] > n.points.length - 1
          )
...
function Jn(e, r, t, a) {
  for (var n = new ue.GlyphSet(a), s = 0; s < t.length - 1; s += 1) {
    var i = t[s],
      u = t[s + 1]
    i !== u
      ? n.push(s, ue.ttfGlyphLoader(a, s, ea, e, r + i, ta))
      : n.push(s, ue.glyphLoader(a, s))
  }
  return n
}
function jn(e, r, t, a) {
...
var ua = function (e) {
  ...
  oe = {
    ...
    interpolate: function (e, r, t, a) {
      ...
      if (
        ((n = a.distance(e, r, !0, !0)),
        (s = a.distance(e, t, !0, !0)),
        (o = a.distance(r, r, !1, !0)),
        (l = a.distance(t, t, !1, !0)),
        (i = Math.abs(n)),
        (u = Math.abs(s)),
        (f = i + u),
        f === 0)
...
Ie.prototype.interpolate = function (e, r, t, a) {
  var n, s, i, u, o, l, f
  if (
    ((i = a.distance(e, r, !0, !0)),
    (u = a.distance(e, t, !0, !0)),
    (n = a.distance(r, r, !1, !0)),
    (s = a.distance(t, t, !1, !0)),
    (o = Math.abs(i)),
    (l = Math.abs(u)),
    (f = o + l),
    f === 0)
...
function Me(e, r) {
  ...
  return (
    (e /= t),
    (r /= t),
    e === 1 && r === 0 ? oe : e === 0 && r === 1 ? le : new Ie(e, r)
  )
}
function fe(e, r, t, a) {
  ;(this.x = this.xo = Math.round(e * 64) / 64),
    (this.y = this.yo = Math.round(r * 64) / 64),
    (this.lastPointOfContour = t),
    (this.onCurve = a),
...
ia.prototype.exec = function (e, r) {
  ...
  if (!(this._errorState > 2)) {
    ...
    if (!a || a.ppem !== r) {
      ...
      if (!n) {
        ;(de.prototype = ss),
          (n = this._fpgmState = new de('fpgm', t.tables.fpgm)),
          (n.funcs = []),
          (n.font = t),
          exports.DEBUG && (console.log('---EXEC FPGM---'), (n.step = -1))
        try {
          Se(n)
        } catch (l) {
      ...
      ;(de.prototype = n),
        (a = this._prepState = new de('prep', t.tables.prep)),
        (a.ppem = r)
      var s = t.tables.cvt
      if (s)
        for (
          var i = (a.cvt = new Array(s.length)), u = r / t.unitsPerEm, o = 0;
          o < s.length;
          o++
        )
          i[o] = s[o] * u
      ...
      try {
        ...
        this._errorState < 2 && console.log('Hinting error in PREP:' + l),
          (this._errorState = 2)
      }
    }
...
sa = function (e, r) {
  ...
  if (((de.prototype = r), !n))
    ...
    for (var l = 0; l < n.length; l++) {
      ...
      ;(u = new de('glyf', h.instructions)),
        exports.DEBUG &&
          (console.log('---EXEC COMP ' + l + '---'), (u.step = -1)),
        br(h, u, t, a)
      for (
        var p = Math.round(f.dx * t),
          c = Math.round(f.dy * a),
          d = u.gZone,
          x = u.contours,
          m = 0;
    ...
    e.instructions &&
      !u.inhibitGridFit &&
      ((u = new de('glyf', e.instructions)),
      (u.gZone = u.z0 = u.z1 = u.z2 = i),
      (u.contours = s),
      i.push(new fe(0, 0), new fe(Math.round(e.advanceWidth * t), 0)),
...
br = function (e, r, t, a) {
  for (
    ...
    (o = n[l]),
      (i[l] = new fe(o.x * t, o.y * a, o.lastPointOfContour, o.onCurve))
  for (var f, h, p = 0; p < s; p++)
    (o = i[p]),
      f || ((f = o), u.push(p)),
      o.lastPointOfContour
        ? ((o.nextPointOnContour = f), (f.prevPointOnContour = o), (f = void 0))
        : ((h = i[p + 1]),
          (o.nextPointOnContour = h),
          (h.prevPointOnContour = o))
  if (!r.inhibitGridFit) {
    if (exports.DEBUG) {
...
function et(e, r) {
  exports.DEBUG && console.log(r.step, 'SVTCA[' + e.axis + ']'),
    (r.fv = r.pv = r.dpv = e)
}
function rt(e, r) {
  exports.DEBUG && console.log(r.step, 'SPVTCA[' + e.axis + ']'),
    (r.pv = r.dpv = e)
}
function tt(e, r) {
...
function at(e, r) {
  ...
  var u, o
  e ? ((u = s.y - i.y), (o = i.x - s.x)) : ((u = i.x - s.x), (o = i.y - s.y)),
    (r.pv = r.dpv = Me(u, o))
}
function nt(e, r) {
  var t = r.stack,
...
function is(e) {
  var r = e.stack,
    t = r.pop(),
    a = r.pop()
  exports.DEBUG && console.log(e.step, 'SPVFS[]', t, a),
    (e.pv = e.dpv = Me(a, t))
}
function os(e) {
  var r = e.stack,
...
function ys(e) {
  var r = e.stack.pop()
  switch (
    (exports.DEBUG && console.log(e.step, 'SZPS[]', r),
    (e.zp0 = e.zp1 = e.zp2 = r),
    r)
  ) {
    case 0:
...
function Rs(e) {
  ...
  for (var i = 0; i < a; i++)
    Se(e),
      exports.DEBUG &&
        console.log(++e.step, i + 1 < a ? 'next loopcall' : 'done loopcall', i)
  ;(e.ip = n), (e.prog = s)
}
function ws(e) {
  var r = e.stack.pop()
  ...
  ;(e.prog = e.funcs[r]),
    ...
    (e.ip = t),
    (e.prog = a),
    exports.DEBUG && console.log(++e.step, 'returning from', r)
}
function Ds(e) {
  var r = e.stack,
    t = r.pop()
...
function As(e) {
  var r = e.stack,
    t = r.pop()
  exports.DEBUG && console.log(e.step, 'MINDEX[]', t),
    r.push(r.splice(r.length - t, 1)[0])
}
function Bs(e) {
  ...
  for (var u = 0; u < a; u++)
    (n = t[u]),
      !e.touched(n) &&
        ...
        s !== n &&
          ((i = n.nextTouched(e)),
          s === i && e.setRelative(n, n, e.distance(s, s, !1, !0), e, !0),
          e.interpolate(n, s, i, e)))
}
function ot(e, r) {
  for (
    var t = r.stack,
    ...
    s.setRelative(f, f, h, i),
      s.touch(f),
      exports.DEBUG &&
        console.log(
          r.step,
          (r.loop > 1 ? 'loop ' + (r.loop - u) + ': ' : '') +
            'SHP[' +
            (e ? 'rp1' : 'rp2') +
            ']',
          l
...
function Is(e) {
  for (
    var r = e.stack, t = e.loop, a = e.fv, n = r.pop() / 64, s = e.z2;
    t--;
...
function Ms(e) {
  for (
    ...
    exports.DEBUG &&
      console.log(
        e.step,
        (e.loop > 1 ? 'loop ' + (e.loop - n) + ': ' : '') + 'IP[]',
        f,
        t,
        '<->',
        a
      ),
      u.interpolate(h, s, i, o),
      u.touch(h)
  }
  e.loop = 1
}
function ft(e, r) {
  var t = r.stack,
    ...
    u = r.fv,
    o = r.pv
  u.setRelative(s, i, a, o),
    u.touch(s),
    exports.DEBUG && console.log(r.step, 'MSIRP[' + e + ']', a, n),
    (r.rp1 = r.rp0),
    (r.rp2 = n),
    e && (r.rp0 = n)
}
function Ps(e) {
  for (
    ...
    exports.DEBUG &&
      console.log(
        e.step,
        (e.loop > 1 ? 'loop ' + (e.loop - n) + ': ' : '') + 'ALIGNRP[]',
        o
      ),
      s.setRelative(l, a, 0, i),
      s.touch(l)
  }
  e.loop = 1
}
...
function pt(e, r) {
  ...
  e && (Math.abs(l - o) < r.cvCutIn && (l = o), (l = r.round(l))),
    i.setRelative(s, De, l, u),
    r.zp0 === 0 && ((s.xo = s.x), (s.yo = s.y)),
    i.touch(s),
    (r.rp0 = r.rp1 = n)
}
function Ns(e) {
...
function ht(e, r) {
  var t = r.stack,
    a = t.pop(),
    n = r.z2[a]
  exports.DEBUG && console.log(r.step, 'GC[' + e + ']', a),
    t.push(r.dpv.distance(n, De, e, !1) * 64)
}
function ct(e, r) {
  var t = r.stack,
    a = t.pop(),
  ...
  exports.DEBUG && console.log(r.step, 'MD[' + e + ']', a, n, '->', u),
    r.stack.push(Math.round(u * 64))
}
function qs(e) {
...
function $s(e) {
  var r = e.stack,
    t = r.pop()
  exports.DEBUG && console.log(e.step, 'ODD[]', t),
    r.push(Math.trunc(t) % 2 ? 1 : 0)
}
function ei(e) {
  var r = e.stack,
    t = r.pop()
  exports.DEBUG && console.log(e.step, 'EVEN[]', t),
    r.push(Math.trunc(t) % 2 ? 0 : 1)
}
function ri(e) {
  var r = e.stack.pop()
  exports.DEBUG && console.log(e.step, 'IF[]', r),
    r || (la(e, !0), exports.DEBUG && console.log(e.step, 'EIF[]'))
}
function ti(e) {
...
function ur(e, r) {
  ...
  for (var f = 0; f < a; f++) {
    ...
    if (c === i) {
      var d = (p & 15) - 8
      d >= 0 && d++,
        exports.DEBUG && console.log(r.step, 'DELTAPFIX', h, 'by', d * o)
      var x = l[h]
...
function vi(e) {
  var r = e.stack,
    t = r.pop()
  exports.DEBUG && console.log(e.step, 'FLOOR[]', t),
    r.push(Math.floor(t / 64) * 64)
}
function di(e) {
  var r = e.stack,
    t = r.pop()
  exports.DEBUG && console.log(e.step, 'CEILING[]', t),
    r.push(Math.ceil(t / 64) * 64)
}
function Ve(e, r) {
  var t = r.stack,
    a = t.pop()
...
function gi(e) {
  var r = e.stack,
    t = r.pop(),
    a = r.pop()
  exports.DEBUG && console.log(e.step, 'WCVTF[]', t, a),
    (e.cvt[a] = (t * e.ppem) / e.font.unitsPerEm)
}
function lr(e, r) {
  var t = r.stack,
    a = t.pop(),
  ...
  for (var u = 0; u < a; u++) {
    ...
    if (f === n) {
      ...
      h >= 0 && h++
      var p = h * i
      exports.DEBUG && console.log(r.step, 'DELTACFIX', o, 'by', p),
        (r.cvt[o] += p)
    }
  }
}
function mi(e) {
  ...
  ;(r &= 15),
    r === 0 ? (e.srThreshold = 0) : (e.srThreshold = (r / 8 - 0.5) * t)
}
function yi(e) {
  var r = e.stack.pop()
...
function ki(e) {
  var r = e.stack,
    t = r.pop(),
    a = 0
  exports.DEBUG && console.log(e.step, 'GETINFO[]', t),
    t & 1 && (a = 35),
    t & 32 && (a |= 4096),
    r.push(a)
}
...
function T(e, r, t, a, n, s) {
  var i = s.stack,
    ...
    y,
    C
  ;(m = x = d.distance(h, f, !0, !0)),
    (y = m >= 0 ? 1 : -1),
    (m = Math.abs(m)),
    e && ((C = s.cvt[u]), a && Math.abs(m - C) < s.cvCutIn && (m = C)),
    t && m < p && (m = p),
    a && (m = s.round(m)),
    c.setRelative(h, f, y * m, d),
    c.touch(h),
    exports.DEBUG &&
      console.log(
        s.step,
...
function re(e, r) {
  ;(this.context = e),
    ...
    (this.current = e[r]),
    (this.backtrack = e.slice(0, r)),
    (this.lookahead = e.slice(r + 1))
}
function er(e) {
...
B.prototype.insertToken = function (e, r, t) {
  ...
  return a
    ? (this.tokens.splice.apply(this.tokens, [r, 0].concat(e)),
      t || this.dispatch('insertToken', [e, r]),
      e)
...
B.prototype.updateContextsRanges = function () {
  ...
  for (
    var e = this.tokens.map(function (a) {
        return a.char
      }),
      r = 0;
    r < e.length;
...
B.prototype.setEndOffset = function (e, r) {
  ...
  return (
    (a.rangeId = r + '.' + n.length),
    n.push(a),
    (this.getContext(r).openRange = null),
    a
  )
}
  ...
  this.contextCheckers.forEach(function (a) {
    ...
    if (
      (!s &&
        a.checkStart(e) &&
        ((s = new Lr(t, null, n)),
        (r.getContext(n).openRange = s),
        r.dispatch('contextStart', [n, t])),
      !!s && a.checkEnd(e))
    ) {
...
function Bi(e, r) {
  var t =
    r.inputCoverage.length +
    r.lookaheadCoverage.length +
  ...
  var l = new re(o, 0),
    ...
    h =
      a.length === r.inputCoverage.length &&
      u.length === r.lookaheadCoverage.length &&
      f.length === r.backtrackCoverage.length,
    p = []
  if (h)
    for (var c = 0; c < r.lookupRecords.length; c++)
      for (
        var d = r.lookupRecords[c],
          x = d.lookupListIndex,
...
J.prototype.lookupFeature = function (e) {
  var r = e.contextParams,
    t = r.index,
    a = this.getFeature({ tag: e.tag, script: e.script })
  if (!a)
    return new Error(
      "font '" +
  ...
  for (
    var n = this.getFeatureLookups(a), s = [].concat(r.context), i = 0;
    i < n.length;
    i++
  )
    for (
      var u = n[i], o = this.getLookupSubtables(u), l = 0;
      l < o.length;
      l++
    ) {
      var f = o[l],
      ...
      switch (h) {
        case '11':
          ;(c = p(r.current)),
            c && s.splice(t, 1, new be({ id: 11, tag: e.tag, substitution: c }))
          break
        case '12':
          ;(c = p(r.current)),
            c && s.splice(t, 1, new be({ id: 12, tag: e.tag, substitution: c }))
          break
        case '63':
          ;(c = p(r)),
            Array.isArray(c) &&
              c.length &&
              s.splice(t, 1, new be({ id: 63, tag: e.tag, substitution: c }))
          break
        case '41':
          ;(c = p(r)),
            c && s.splice(t, 1, new be({ id: 41, tag: e.tag, substitution: c }))
          break
        case '21':
          ;(c = p(r.current)),
            c && s.splice(t, 1, new be({ id: 21, tag: e.tag, substitution: c }))
          break
      }
...
J.prototype.getFeature = function (r) {
  ...
  return t
    ? t.tags[r.tag]
      ? this.features[r.script].tags[r.tag]
      : null
...
function ro() {
  return (
    pr.call(this, 'latinWord'),
    pr.call(this, 'arabicWord'),
    pr.call(this, 'arabicSentence'),
...
ne.prototype.registerFeatures = function (e, r) {
  ...
  this.featuresTags.hasOwnProperty(e)
    ? (this.featuresTags[e] = this.featuresTags[e].concat(a))
    : (this.featuresTags[e] = a)
}
ne.prototype.applyFeatures = function (e, r) {
  ...
  for (var t = 0; t < r.length; t++) {
    var a = r[t]
    !this.query.supports({ script: a.script }) ||
...
ne.prototype.applyFeaturesToContexts = function () {
  this.checkContextReady('arabicWord') && (ao.call(this), no.call(this)),
    this.checkContextReady('latinWord') && so.call(this),
    this.checkContextReady('arabicSentence') && to.call(this)
}
ne.prototype.processText = function (e) {
  ;(!this.text || this.text !== e) &&
    (this.setText(e), ro.call(this), this.applyFeaturesToContexts())
}
...
function w(e) {
  ;(e = e || {}),
    ...
    e.empty ||
      ...
      Ee(e.ascender, 'When creating a new Font object, ascender is required.'),
      Ee(
        e.descender <= 0,
    ...
    (this.encoding = new Ot(this)),
    (this.position = new Be(this)),
    (this.substitution = new K(this)),
...
w.prototype.stringToGlyphs = function (e, r) {
  ...
  var s = r
    ? this.updateFeatures(r.features)
    : this.defaultRenderOptions.features
  a.applyFeatures(this, s)
  for (
    var i = a.getTextGlyphs(e),
      u = i.length,
      o = new Array(u),
      l = this.glyphs.get(0),
      f = 0;
...
w.prototype.getKerningValue = function (e, r) {
  ...
  return t
    ? this.position.getKerningValue(t, e, r)
    : this.kerningPairs[e + ',' + r] || 0
}
...
w.prototype.forEachGlyph = function (e, r, t, a, n, s) {
  ;(r = r !== void 0 ? r : 0),
    (t = t !== void 0 ? t : 0),
    (a = a !== void 0 ? a : 72),
    (n = Object.assign({}, this.defaultRenderOptions, n))
  ...
  for (var f = 0; f < u.length; f += 1) {
    var h = u[f]
    if (
      (s.call(this, h, r, t, a, n),
      h.advanceWidth && (r += h.advanceWidth * i),
      n.kerning && f < u.length - 1)
    ) {
      var p = o
        ? this.position.getKerningValue(o, h.index, u[f + 1].index)
        : this.getKerningValue(h, u[f + 1])
      r += p * i
    }
    n.letterSpacing
      ? (r += n.letterSpacing * a)
      : n.tracking && (r += (n.tracking / 1e3) * a)
  }
  return r
}
w.prototype.getPath = function (e, r, t, a, n) {
  var s = new P()
  return (
    this.forEachGlyph(e, r, t, a, n, function (i, u, o, l) {
...
te[1] = function () {
  ...
  U.assert(
    !1,
    '0x' + r.toString(16) + ': GPOS lookup type 1 format must be 1 or 2.'
  )
}
te[2] = function () {
  var r = this.offset + this.relativeOffset,
  ...
  U.assert(
    t === 1 || t === 2,
    '0x' + r.toString(16) + ': GPOS lookup type 2 format must be 1 or 2.'
  )
...
function So(e, r) {
  ...
  var t = new v(e, r),
    a = t.parseVersion(1)
  return (
    U.argument(a === 1 || a === 1.1, 'Unsupported GPOS table version ' + a),
    a === 1
      ? {
          version: a,
          scripts: t.parseScriptList(),
...
function ko(e) {
  return new b.Table('GPOS', [
    ...
    { name: 'scripts', type: 'TABLE', value: new b.ScriptList(e.scripts) },
    { name: 'features', type: 'TABLE', value: new b.FeatureList(e.features) },
    { name: 'lookups', type: 'TABLE', value: new b.LookupList(e.lookups, To) }
  ])
}
...
function Uo(e) {
  ...
  e.skip('uShort')
  var t = e.parseUShort()
  U.argument(t === 0, 'Unsupported kern sub-table version.'),
    e.skip('uShort', 2)
  var a = e.parseUShort()
  e.skip('uShort', 3)
  for (var n = 0; n < a; n += 1) {
...
function Co(e) {
  ...
  e.skip('uShort')
  var t = e.parseULong()
  t > 1 && console.warn('Only the first kern subtable is supported.'),
    e.skip('uLong')
  var a = e.parseUShort(),
    n = a & 255
...
function Lo(e, r, t, a) {
  for (
    var n = new k.Parser(e, r),
      s = a ? n.parseUShort : n.parseULong,
      i = [],
      u = 0;
    u < t + 1;
    u += 1
...
function Do(e, r) {
  ...
  t.open('get', e, !0),
    ...
    (t.onload = function () {
      return t.response
        ? r(null, t.response)
        : r('Font could not be loaded: ' + t.statusText)
    }),
    (t.onerror = function () {
...
function yt(e, r) {
  for (var t = [], a = 12, n = 0; n < r; n += 1) {
    var s = k.getTag(e, a),
      ...
      u = k.getULong(e, a + 8),
      o = k.getULong(e, a + 12)
    t.push({ tag: s, checksum: i, offset: u, length: o, compression: !1 }),
      (a += 16)
  }
  return t
}
...
function Dr(e, r) {
  ...
  for (var f, h, p, c, d, x, m, y, C, S, R, O, D = 0; D < i; D += 1) {
    ...
    switch (L.tag) {
      case 'cmap':
        ;(F = I(s, L)),
          (n.tables.cmap = Et.parse(F.data, F.offset)),
          (n.encoding = new Lt(n.tables.cmap))
        break
      case 'cvt ':
        ;(F = I(s, L)),
          (O = new k.Parser(F.data, F.offset)),
          (n.tables.cvt = O.parseShortList(L.length / 2))
        break
      case 'fvar':
        h = L
        break
      case 'fpgm':
        ;(F = I(s, L)),
          (O = new k.Parser(F.data, F.offset)),
          (n.tables.fpgm = O.parseByteList(L.length))
        break
      case 'head':
        ;(F = I(s, L)),
          (n.tables.head = Nt.parse(F.data, F.offset)),
          (n.unitsPerEm = n.tables.head.unitsPerEm),
          (t = n.tables.head.indexToLocFormat)
        break
      case 'hhea':
        ;(F = I(s, L)),
          (n.tables.hhea = Ht.parse(F.data, F.offset)),
      ...
      case 'maxp':
        ;(F = I(s, L)),
          (n.tables.maxp = _t.parse(F.data, F.offset)),
      ...
      case 'post':
        ;(F = I(s, L)),
          (n.tables.post = Qt.parse(F.data, F.offset)),
          (n.glyphNames = new Ur(n.tables.post))
        break
      case 'prep':
        ;(F = I(s, L)),
          (O = new k.Parser(F.data, F.offset)),
          (n.tables.prep = O.parseByteList(L.length))
        break
      case 'glyf':
        p = L
        break
  ...
  var G = I(s, S)
  if (
    ((n.tables.name = Zt.parse(G.data, G.offset, a)),
